...
public class EntityComponentSystem
{
    ...
    private List<ComponentArrayBase> componentArrays = new List<ComponentArrayBase>();
    ...
    public void RemoveEntity(int entity)
    {
        ...
        foreach (var compArr in componentArrays)
            compArr.OnEntityRemoved(entity);
    }
    ...
    {
        int compTypeIndex = ComponentIndex<T>.guid;
        ComponentArray<T> compArr;
        if (compTypeIndex >= componentArrays.Count)
        {
            compArr = new ComponentArray<T>();
            compArr.components = Enumerable.Repeat(default(T), maxEntities).ToList();
            componentArrays.Add(compArr);
        }
        else
        {
            compArr = (ComponentArray<T>)componentArrays[compTypeIndex];
        }
        return compArr;
    }
}
